...
export const getBlogPosts = ():Blog[]=> {
  ...
  for (const langDir of Object.values(AVAILABLE_LOCALES)) {
    ...
    files.forEach((file) => {
      ...
      allBlogs.push({ slug,lang:langDir, content,title:frontMatter.title,description:frontMatter.description,createdAt:frontMatter.createdAt,image:frontMatter.image,fileNamea:frontMatter.fileNamea, })
    })
  }
  return allBlogs
}
